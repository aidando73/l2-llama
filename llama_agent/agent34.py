...
def run_agent(
    ...
    for i in range(PHASE2_ITERATIONS):
        ...
        while old_content_match := re.search(r"<old_content>(.*?)</old_content>", response_content, re.DOTALL):
            ...
            try:
                new_content = replace_content(file_content, old_content, new_content)
            except Exception as e:
                msg = f"ERROR - edit {i} - {e}"
                print("System: " + red(msg))
                message += chat_message("system", msg)
                continue
...
def replace_content(old_file_content: str, old_content: str, new_content: str):
    ...
    if dedent(old_content) == dedent(new_content):
        raise AssertionError("old_content and new_content are the same. Please ensure that new_content is different from old_content.")
