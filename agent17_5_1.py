...
from ansi import red, green, magenta
...
def run_agent(
    ...
    for i in range(ITERATIONS):
        ...
        if thinking_match:
            print(magenta(f"Thinking: {thinking_match.group(1).strip()}"))
        else:
            ...
            if non_tool_content:
                print(magenta(f"Thinking: {non_tool_content}"))
        ...
        for tool_call in tool_calls:
            ...
            if result == "success":
                print("Result: " + green(result_msg))
            else:
                print("Result: " + red(result_msg))
    ...
    # Create a PR title
    message += chat_message("user", "Please create a PR title that summarizes the changes you've made. Do not include any leading or trailing punctuation.")
    message += header("assistant")
    response = client.inference.completion(
        model_id=MODEL_ID,
        content=message,
    ...
    diff_cmd = run(f"cd sandbox/{repo} && git diff")
...
def execute_tool_call(tool_name: str, tool_params: dict[str, str]) -> Union[Tuple[Literal["success"], str], Tuple[Literal["error"], str]]:
    """
    ...
    if tool_name == "list_files":
        ...
        try:
            ...
            return ("error", f"ERROR - Directory not found: {e}")


    elif tool_name == "edit_file":
        if error := validate_param_exists("path", tool_params):
            return ("error", error)
        if error := validate_param_exists("new_str", tool_params):
            return ("error", error)
        try:
            path = translate_path(tool_params["path"])
            ...
            return ("error", f"ERROR - File {tool_params['path']} not found. Please ensure the path is an absolute path and that the file exists.")

        except IsADirectoryError:
            return ("error", f"ERROR - Path {tool_params['path']} is a directory. Please ensure the path references a file, not a directory.")
...
def validate_param_exists(param_name: str, tool_params: dict[str, str]) -> Optional[str]:
