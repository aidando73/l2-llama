...
TOOLS = [
    ...
    ToolDefinition(
        ...
        parameters={
            ...
            "insert_line": ToolParamDefinition(
                param_type="number",
                description=("The line number to insert the new_str at. "
                             "Will insert new_str after the specified line number. "
                             "Line numbers start at 1. Specify line number 0 to insert at the start of the file. "
                             "If not specified, then old_str must be specified."),
                required=False,
            ),
            "old_str": ToolParamDefinition(
                param_type="string",
                description="The string in the file at `path` to replace. If not specified, then insert_line must be specified.",
                required=False,
            ),
            "new_str": ToolParamDefinition(
                param_type="string",
                description=("The new string to write to the file. "
                            "If the old_str is specified, only the old_str will be replaced with new_str. "
                            "If insert_line is specified, then new_str will be inserted at the specified line number."
                        ),
                required=True,
            ),
        },
...
def execute_tool_call(
    ...
    if tool_name == "list_files":
        ...
        elif "insert_line" in tool_params:
            with open(f"{path}", "r") as f:
                file_content = f.read()
